...
src_install() {
	...
	if use bmpx; then
		exeinto ${ROOT}usr/$(get_libdir)/bmpx/plugins/imms
		doexe build/libbmpimms*.so || die "install bmplib failed"
	fi

	if use audacious; then
		exeinto ${ROOT}usr/$(get_libdir)/audacious/General
		doexe build/libaudaciousimms*.so \
			|| die "install audaciouslib failed"
	fi

	if use quodlibet; then
	   insinto ${ROOT}usr/$(get_libdir)/python${PYVER}/site-packages/quodlibet/plugins
...
pkg_postinst() {
	python_version
	python_mod_optimize ${ROOT}usr/$(get_libdir)/python${PYVER}/site-packages/quodlibet/plugins
}
