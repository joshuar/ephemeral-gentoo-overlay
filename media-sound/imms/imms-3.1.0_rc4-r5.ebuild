...
pkg_postinst() {
	if use quodlibet; then
		python_version
		python_mod_compile /usr/$(get_libdir)/python${PYVER}/site-packages/quodlibet/plugins/events/imms.py
	fi
}

pkg_postrm() {
	if use quodlibet; then
		python_mod_cleanup /usr/$(get_libdir)/python${PYVER}/site-packages/quodlibet/plugins/events
	fi
}
