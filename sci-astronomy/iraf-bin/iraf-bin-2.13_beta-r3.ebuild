...
pkg_setup() {
	# various lists of things used later
	...
	# create links for base iraf archives so unpack
	#  understands they are really tar.gz files.
	ln -sf "${DISTDIR}"/as.pcix.gen.gz "${DISTDIR}"/as.pcix.gen.tar.gz
	ln -sf "${DISTDIR}"/ib.rhux.x86.gz "${DISTDIR}"/ib.rhux.x86.tar.gz
	ln -sf "${DISTDIR}"/nb.rhux.x86.gz "${DISTDIR}"/nb.rhux.x86.tar.gz
...
src_compile() {
	...
	if use pyraf; then
		...
		distutils_src_compile --with-f2c=/usr
	fi
}
